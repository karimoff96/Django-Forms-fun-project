...
def order(request):
    ...
    if request.method == 'POST':
        ...
        if filled_form.is_valid():
            ...
            note = 'Your %s and %s order %s on its way' % (filled_form.cleaned_data['size'],
                                                           filled_form.cleaned_data['topping1'],
                                                           filled_form.cleaned_data['topping2'],)
            filled_form = PizzaForm()
        else:
            created_pizza_pk=None
            note = 'Pizza order has failed. Try egain'
...
def pizzas(request):
    ...
    if request.method == 'POST':
        ...
        if filled_formset.is_valid():
            ...
            note = 'Pizzas have been arrieved'
        else:
            note = 'Order was not created, please ty again'
